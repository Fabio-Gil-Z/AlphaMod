            ...
            if key in template:
                for target in alphafold_or_modeller_results_path:
                    if key in target:
                        ...
                        MOLPROBITY = configuration["StructureAssessment"]["MOLPROBITY"]
                        while True:
                            subprocess.check_call(f'python3 {Google_Chrome_Selenium_Web_Bot_Folder}/main.py -Q false -P false -M {MOLPROBITY} -C false -T {target}', shell=True, cwd=cwd)
                            MOLPROBITY_result_files = glob.glob(f"{Google_Chrome_Selenium_Web_Bot_Folder}/MOLPROBITY/*")
                            # If everything went good, we will have 6 files and no errors otherwise we keep trying.
                            if len(MOLPROBITY_result_files) == 6:
                                break
                        ...
                        MOLPROBITY_result_files = glob.glob(f"{Google_Chrome_Selenium_Web_Bot_Folder}/MOLPROBITY/*")
                        for file in MOLPROBITY_result_files:
...
def deletePDB():
    ...
    for pdb in pdb_files_to_delete:
        os.remove(pdb)
def MOLPROBITY(cwd, configuration):
    cwd = os.getcwd()
    ...
    output_folder = f'{cwd}/Structure_Assessment_Section/MOLPROBITY/results'
    ...
    os.rmdir(f"{Google_Chrome_Selenium_Web_Bot_Folder}/MOLPROBITY")

def main():
    cwd = os.getcwd()
    configuration = readConfigJSON(cwd)
    MOLPROBITY(cwd, configuration)

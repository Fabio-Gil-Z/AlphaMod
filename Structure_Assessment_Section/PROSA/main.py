            ...
            if key in template:
                for target in alphafold_or_modeller_results_path:
                    if key in target:
                        ...
                        for file in PROSA_result_files:
...
def PROSA(cwd, configuration):
    ...
    output_folder = f'{cwd}/Structure_Assessment_Section/PROSA/results'
    ...
    os.rmdir(f"{Google_Chrome_Selenium_Web_Bot_Folder}/PROSA")

def main():
    cwd = os.getcwd()
    configuration = readConfigJSON(cwd)
    PROSA(cwd, configuration)
